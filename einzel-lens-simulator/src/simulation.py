...
class Chip:
    ...
    def get_aperture_focal_length(
        self, V_left, d_left, V_lens, d_right, V_right, base_voltage
    ):
    ...
    def get_all_focal_lengths(self, voltages):
        ...
        for aperture_index, spacing in enumerate(self.spacings):
            ...
            if aperture_index < len(self.spacings) - 1:
                focal_lengths[aperture_index] = self.get_aperture_focal_length(
                    voltages[aperture_index],
                    spacing,
    ...
    def transform_deflection_aperture(
        self, incident_ray_deflection_angle, incident_ray_offset, aperture_focal_length
    ):

        outgoing_deflection_angle = (
            incident_ray_deflection_angle - incident_ray_offset / aperture_focal_length
        )
    ...
    def transform_deflection_gap(
        self,
        incident_ray_deflection_angle,
        incident_ray_offset,
        bounding_voltages,
        plate_separation,
    ):
        ...
        """
        try:
            outgoing_deflection_angle = incident_ray_deflection_angle * np.sqrt(
                bounding_voltages[0] / bounding_voltages[1]
            )
        except ZeroDivisionError:
        ...
        try:
            outgoing_ray_offset = (
                ...
                * 2
                * plate_separation
                / (np.sqrt(bounding_voltages[1] / bounding_voltages[0]) + 1)
            )
        except ZeroDivisionError:
            outgoing_ray_offset = 0
        return outgoing_deflection_angle, outgoing_ray_offset

    def get_all_offsets_and_deflections(
        self,
        electron_release_angle,
        electron_release_offset,
        electron_release_energy,
        voltages,
    ):
        deflections, offsets = np.zeros(len(self.spacings) + 1), np.zeros(
            len(self.spacings) + 1
        )
        ...
        for aperture_index, spacing in enumerate(self.spacings):
            if aperture_index == len(
                self.spacings
            ):  # TODO: Adjust for odd number of spacings!
                break
            else:
                deflection_before_aperture, offset_before_aperture = (
                    self.transform_deflection_gap(
                        deflections[aperture_index],
                ...
                deflections[aperture_index + 1], offsets[aperture_index + 1] = (
                    self.transform_deflection_aperture(
                        deflection_before_aperture,
                        offset_before_aperture,
                        focal_lengths[aperture_index],
                    )
                )
                if (
                    deflections[aperture_index + 1] > 1
                    or deflections[aperture_index + 1] < -1
                ):
                    print("The paraxial approximation has been violated:")
                    print(
                        "Deflection angle encountered: "
                        + str(deflections[aperture_index + 1])
                        + " rad"
                    )
    ...
    def linear_ray_path(self, deflections, offsets, number_of_datapoints):
        ...
        for spacing_index, spacing in enumerate(self.spacings):
            ...
            values = (
                x[
                    np.where(
                        (x >= distance_into_the_system)
                        & (x < distance_into_the_system + spacing)
                    )
                ]
                - distance_into_the_system
            ) * (
                offsets[spacing_index + 1] - offsets[spacing_index]
            ) / spacing + offsets[
                spacing_index
            ]
            y += values.tolist()
        ...
        values = (
            x[np.where((x >= distance_into_the_system))] - distance_into_the_system
        ) * deflections[-1] + offsets[-1]
        y += values.tolist()
    ...
    def plot_custom_ray(
        ...
        """
        deflections, offsets = self.get_all_offsets_and_deflections(
            electron_release_angle,
            electron_release_offset,
            electron_release_energy,
            voltages,
        ...
        for spacing_index, spacing in enumerate(self.spacings):
            ...
            plt.axvline(
                ...
                alpha=0.5,
                dashes=([2, 4, 2, 4]),
                label=f"Aperture {spacing_index + 1}, ${voltages[spacing_index]} V$",
            )
...
chip_v0 = Chip([2e-3, 2e-3, 500e-9, 500e-9], [50e-9, 50e-9, 50e-9, 50e-9], 250e-9)
